...
PLAYER_MOVE_PIX = 5 #перемещение игрока на каждую итерацию (пиксели)
...
while True: #этот цикл нужен чтобы отображать окно, пока игра не завершена
    ...
    moveLeft = moveRight = moveUp = moveDown = False #клавиша не нажата с самого начала
    enemyAddCounter = 0  #счетчик добавления врагов 
    ...
    while True: #игровой цикл
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN: #условие нажатия клавиш
            ...
            if event.type == KEYUP: #условие отпускания клавиш
        ...
        if moveLeft and playerRect.left > 0: #движения игрока
            playerRect.move_ip(-1 * PLAYER_MOVE_PIX, 0)
        if moveRight and playerRect.right < WINDOWWIDTH:
            playerRect.move_ip(PLAYER_MOVE_PIX, 0)
        if moveUp and playerRect.top > 0:
            playerRect.move_ip(0, -1 * PLAYER_MOVE_PIX)
        if moveDown and playerRect.bottom < WINDOWHEIGHT:
            playerRect.move_ip(0, PLAYER_MOVE_PIX)
